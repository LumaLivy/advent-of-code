...
const argv = getArgs(process.argv.slice(2), "2022", "06");
...
const partOne = (data) => {
  ...
  return (
    1 +
    cleaned.findIndex((s, si) => {
      if (si < 3) return;
      const set = [...new Set([...cleaned.slice(si - 3, si + 1)])];
      return set.length === 4;
    })
  );
};
...
const partTwo = (data) => {
  ...
  return (
    ...
    cleaned.findIndex((s, si) => {
      if (si < 13) return;
      const set = [...new Set([...cleaned.slice(si - 13, si + 1)])];
